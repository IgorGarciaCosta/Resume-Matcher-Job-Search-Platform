...
public class HimalayasJobSearchProvider : IJobSearchProvider
{
    ...
    public async Task<JobSearchResponseDto> SearchAsync(JobSearchRequestDto request, CancellationToken ct = default)
    {
        ...
        {
            var salary = SalaryFormatter.FormatSalary(j.MinSalary, j.MaxSalary, j.Currency);
            ...
            return new JobSearchResultDto
            {
                ...
                Location = j.LocationRestrictions is { Count: > 0 }
                    ...
                    : "Remote",
                Description = HtmlHelper.StripHtml(j.Description ?? j.Excerpt ?? string.Empty),
